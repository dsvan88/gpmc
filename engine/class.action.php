...
class Action {
	...
	function DBInit()
	{
		...
		$this->query(
			str_replace('{SQL_TBLNEWS}', SQL_TBLNEWS,
				"CREATE TABLE IF NOT EXISTS {SQL_TBLNEWS} (
					...
					date_add TIMESTAMP DEFAULT NULL,
		...
		if (!$this->recordExists(['id'=>1],SQL_TBLUSERS)){
			$this->rowInsert([
				'login'=> 'admin',
				'password' => '$2y$10$nrFt674uOjlgALrZBfu4nu6a4f8bX1h/rmoe2A.2kPUNspmObnl4q',
				'status' => 'admin',
				'admin' => '1'
			],SQL_TBLUSERS);
		}
		if ($this->getColumn($this->query('SELECT COUNT (id) FROM '.SQL_TBLSETTINGS)) < 1){
			$setting = [
				...
				[ 'txt', 'about-game', 'Об игре', 'Игра очень интересная! Рекомендую!:)', 'Клубная игра Мафия, в классическом исполнении поражает простотой и одновременной сложностью, ведь результат всегда зависит не только от личного вклада каждого отдельного игрока, но и в целом команды... а вот кто находится в твоей команде - интрига сохраняется до самого конца игры! Прокачайте свои навыки логики, дедукции, эмпатии, интуиции, да и телепатии, вместе с нами!']
			];
			$array = [];
