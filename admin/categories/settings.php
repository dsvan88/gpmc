	...
	<div id='TxtTable'>
		<?for($x=0;$x<count($a_settings);$x++):?>
			<div class='TxtTableCell'>
				<div class='TxtCellTop'><b><?=$a_settings[$x]['shname']?></b><a class='EditPencil' id="<?=$a_settings[$x]['id']?>"><img src = '<?=$settings['img']['edit_pen']['value']?>' title='Изменить' alt='Изменить'/></a></div>
	...
	<div id='PntTable'>
		<?for($x=0;$x<count($a_settings);$x++):?>
			<div class='PntTableCell' id="<?=$a_settings[$x]['id']?>">
				<span class='point_name'><b><?=$a_settings[$x]['name']?></b></span><span class="point_value"><?=str_replace(',',', ',$a_settings[$x]['value'])?></span><a class='EditPencil'><img src = '<?=$settings['img']['edit_pen']['value']?>' title='<?=$settings['img']['edit_pen']['name']?>' alt='<?=$settings['img']['edit_pen']['name']?>'/></a>
			</div>
		<?endfor?>
	</div>
<?elseif ($_GET['p']==='users'):
	...
	<table id='UsrTable' style="border-collapse:collapse">
		...
		<?for($x=0;$x<count($users);$x++):?>
			<tr id='<?=$users[$x]['id']?>'>
				...
				<td><a class='EditPencil'><img src = '<?=$settings['img']['edit_pen']['value']?>' title='<?=$settings['img']['edit_pen']['name']?>' alt='<?=$settings['img']['edit_pen']['name']?>'/></a></td>
			</tr>
		<?endfor?>
		</tbody>
	</table>
