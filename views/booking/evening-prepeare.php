...
if (!isset($EveningData['participants_info']) && !(isset($_SESSION['status']) && in_array($_SESSION['status'],['admin','manager','founder','master']))){
	$output['{EVENING_SECTION}'] = file_get_contents( $_SERVER['DOCUMENT_ROOT'].'/templates/booking/booking-none.html');
}
else{
	...
	$output['{EVENING_SECTION}'] = file_get_contents($htmlFiles['booking']);
