...
$id = $engine->GetPlayerID($_POST['player_name'],1);
if ($id === false)
{
	$result['error'] = 1;
...
$player_data = $engine->GetPlayerData(array('username'),array('username'=>$_POST['username']));
...
$player_data = $engine->GetPlayerData(array('username'),array('id'=>$id));
...
$engine->UpdateRow($a,array('id'=>$id),MYSQL_TBLPLAYERS);
exit(json_encode($result,JSON_UNESCAPED_UNICODE));